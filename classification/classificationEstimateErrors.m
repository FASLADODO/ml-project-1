...
% degree = 0.5;
% X = [X(:, 1:29) X(:, 30: end)];
% X_te = [X_te(:, 1:29) X_te(:, 30: end)];


% % Normalize features (except the discrete ones)
% notnormalized = X(:,1:29);
% [X(:,1:29), X(:,30:58)] = normalized(X(:,1:29), X(:,30:58));
% [~, X_te(:,1:29)] = normalized(notnormalized, X_te(:,1:29));
% [~, X_te(:,30:58)] = normalized(notnormalized, X_te(:,30:58));
